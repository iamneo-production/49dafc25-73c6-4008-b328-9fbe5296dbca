...
--Sql query to find the percentage of the female users who are using social media
select(sum(case when gender = 'female' then 1 else 0 end) / count(*)*100)as female_percentage from FACEBOOK;

--Sql query to find the percentage of the female users among users above the age of 17
